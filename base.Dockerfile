...
RUN apt-get update && \
    apt-get install -y curl wget git unzip zsh autojump python3 pip tmux fd-find ripgrep fzf x11-apps \ 
							xsel xclip ca-certificates locales tzdata net-tools lsof ncurses-term tini build-essential gcc g++
...
# 安装 tree-sitter CLI（nvim-treesitter 编译需要）
RUN PATH="/root/.local/bin:$PATH" npm install -g tree-sitter-cli
...
# 安装 claudecode
RUN PATH="$HOME/.local/bin:$PATH" ~/.local/bin/npm config set registry https://registry.npmmirror.com
